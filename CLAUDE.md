...
**외부 데이터 소스의 범위·기본값·필터는 어물쩍 정하지 않는다.** UI 결정은 시안으로 사인오프를
받으면서, 정작 "무엇이 언제 왜 보이나"를 좌우하는 데이터 정책은 코드 짜다 한 줄로 조용히
굳는 비대칭이 있었다 (전국 평균·상품 등급·부류 3개 범위 등을 사용자가 뒤늦게 알게 된 원인).

외부 API를 붙이거나 데이터 파이프라인을 만들 때, 아래에 해당하는 선택은 **코드 짜기 전에 한 줄로
표면화하고 확인받는다** (스펙에 적고, 애매하면 물어본다). 코드는 "무엇을 하는지"는 보여줘도
"무엇을 **안** 하는지"(빠진 카테고리·안 쓴 컬럼·거른 등급)는 안 보여주기 때문이다:

- **범위 필터** — 어떤 카테고리/품목/지역을 받고 무엇을 뺐나 (예: KAMIS 부류 100·200·400만).
- **기본값·모수 선택** — 전국 평균 vs 지역, 소매 vs 도매, 상품 등급 우선, 기준선으로 고른 컬럼.
- **결측·폴백 규칙** — 없는 값을 null로 둘지 다른 값으로 메울지, 며칠까지 소급할지, 실패 시 동작.
- **표시에 영향 주는 임계** — "오래됨" 기준일, "비슷" 임계 % 등.

이 결정들은 대개 화면에 **부재로** 나타나(빠진 게 안 보인다) 나중에 놀라움이 된다. 표면화 =
그 부재를 사용자가 볼 기회를 만드는 것. 판단이 서면 **`docs/제품-동작-지도.md`의 수집 정책·지렛대
지도**와 필요시 **`docs/아이디어-백로그.md`**를 함께 갱신한다.
