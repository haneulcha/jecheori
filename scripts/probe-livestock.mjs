...
/** 부류 500 원시 응답을 parseUnit 없이 열거한다 — 단위·등급 문자열을 **가르치기 전에** 발견하는 게 목적.
 *  parseCategoryResponse는 모르는 단위에 throw하므로 여기선 원시 필드를 직접 읽는다. */
export function summarizeLivestock(json) {
...
/** 부류 500 하루치를 조회해 품목·등급·단위를 요약. fetchFn 주입으로 테스트.
 *  쿼리 파라미터는 fetch-prices.mjs의 buildSnapshot과 동일 기준(소매·전체 평균)을 그대로 따른다. */
export async function probeLivestock({ certKey, certId, regday, fetchFn = fetch }) {
  ...
  url.searchParams.set('p_convert_kg_yn', 'N') // 단위 보존
  ...
  if (!res.ok) throw new Error(`KAMIS HTTP ${res.status} (부류 500)`)
  return summarizeLivestock(await res.json())
}
...
if (isMain) {
  ;(async () => {
    ...
    try {
      const { items, units } = await probeLivestock({ certKey, certId, regday })
      console.log(`부류 500 (${regday}): ${items.length}개 항목, 단위 ${units.length}종 → ${units.join(', ')}`)
