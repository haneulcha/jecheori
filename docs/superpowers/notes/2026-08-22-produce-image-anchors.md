...
**현재: 3회차로 게이트 1 통과. 프롬프트 동결.** 회차별 기록은 아래.
...
고등어가 대각선으로 누워 96px에서 처음으로 "생선"이 아니라 "고등어"로 읽힌다.
네 장의 평탄화 밀도가 한 가족으로 보이고, 크림 종이 위에서도 헤일로가 없다.

**Gemini 워터마크.** 4장 전부 우하단 `(1760,1760)` 96×96 자리에 ✦가 찍혀 있었다.
프롬프트로 막을 수 있는 게 아니라 제품이 사후에 얹는 것. **후처리에서 제거하기로
결정**했고(하늘 판단), 파이프라인이 본체에서 떨어진 1% 미만 조각을 떼면서 무엇을
얼마나 뗐는지 로그로 남긴다. 워터마크는 0.14~0.18%로 찍히고, 2회차 라벨 글자처럼
프롬프트 위반에서 온 조각(1.7~4.6%)엔 경고가 붙는다.

**시각 질량.** 96px에서 양배추·등심이 복숭아·고등어보다 덩치가 커 보인다 — 80%
규칙이 긴 변을 맞춰서 대각선 생선·잎이 벌어진 복숭아는 면적이 작다. **지금대로
두기로 했다**(하늘 판단): 실물도 양배추가 복숭아보다 크니 그 정도 차이는 자연스럽고,
70장을 뽑기 전에 기준을 또 바꾸지 않는다.

**용량 실측:** mackerel 8.0KB · peach 10.5KB · cabbage 13.1KB · hanwoo-sirloin 16.4KB.
평균 **11.9KB** → 70장 ≈ **830KB**.

### 동결

- **생성기:** Gemini 계열 (GPT Image 비교 미수행 — 구독 없음)
- **프롬프트:** 위 4개의 고정 6블록. 이후 66장은 슬롯 4개만 갈아끼운다
- **레퍼런스 첨부:** 아래 문장과 함께 앵커 4장을 매번 첨부
- **날짜:** 2026-08-23
...
### 결과 (2026-08-23) — **좌판 유지. 전환하지 않는다**

`peach` 한 장으로 A/B. 시트: `scratchpad/ab-peach.png`, 브라우저 A/B는 DPR 페이지 2번.

관측된 대로 **트레이드오프는 예상과 정확히 같았다**:

- **도판이 96px에서 형태를 더 잘 지킨다.** 면이 크니 뭉개져도 남는다. 실루엣이 또렷하다.
- **대신 붉은 볼이 '물든 자국'이 아니라 '오려붙인 도형'으로 읽힌다.** 잎도 평평해지고
  전체가 스티커·클립아트에 가까워진다. 색도 창백해 덜 먹음직하다.

**유지 근거 셋:**

1. **도판의 유일한 장점이 Retina에서 절반쯤 사라진다.** "축소를 견딘다"는 1x 기준
   이야기고, 2x·3x에서는 브라우저가 288px 원본에서 그리므로 좌판도 충분히 또렷하다.
   (이 사실 자체가 이번에 확인됐다 — 초기 평가 시트가 96px 사전축소본이라 1x를
   흉내내고 있었고, 그래서 "실사처럼 보인다"는 인상이 나왔다.)
2. **도판은 이모지 쪽으로 반걸음 되돌아간다.** 우리가 떠난 곳이 "납작한 그림 기호"인데
   96px 도판 복숭아가 그 방향이다.
3. **식욕을 잃는다.** 실사 기반을 고른 이유가 그건데, 내주고 얻는 게 Retina에서 잘
   안 보이는 선명도다.

**시범이 남긴 것:** 파이프라인 버그 하나를 잡았다. 잎·꼭지가 고리를 이루자 안쪽 배경이
테두리와 안 이어져 flood fill로 안 지워졌다 — 전역 키 제거로 바꿨다. 갈비뼈 사이·
지느러미 틈·마늘 쪽 사이에서 계속 나올 문제였으니, 이 한 장이 66장을 구했다.
