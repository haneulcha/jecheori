...
## 최종 확정형 (구현 후 브라우저 반복으로 개정)

> 아래 "원장 프레임" 절은 브레인스토밍 시점의 확정안이다. **구현 후 사용자 브라우저 반복**에서
> 원장 격자·해치·도트를 거쳐 **간트 바**로 최종 개정됐다. 실제 shipped 형태는 이 절이 정본:
>
> - **간트 바**: 12개월 축 위에 제철=연함(`--tint`)·절정=짙음(`--accent`) — 격자·외곽 프레임
>   없이 연속 바. 바 높이 `0.45rem`.
> - **축선**: `--line` 가로선이 바 **세로 중앙**을 지난다. 시작·끝점을 1·12월 숫자 중앙에 맞춰
>   양끝 `100%/24`(반 칸)씩 인셋.
> - **숫자**: 제철 월 + 이번 달만 표기(모노, `0.6rem`), 이번 달만 잉크 볼드, 나머지 숨김
>   (span 12개는 유지해 칸 정렬 보존).
> - **배치**: 이모지·이름 하단 왼쪽 열(`id-wrap`, `flex-direction:column`), **반폭(60%)
>   좌측정렬**. `<summary>` 안이라 접힘/펼침 모두 표시(가격 블록 높이와 무관하게 이름에 붙는다).
> - **▼ 이번 달 마커·캡션·원장 프레임은 폐기**. 색 규율(웜=배경, 잉크=숫자)·현재 계절색 상속은 유지.
> - 아키텍처(§ 아래)는 그대로 유효: `toSeasonStrip` 순수 파생 + `SeasonStrip` 표시.

## 확정 시안 — 원장 프레임 + 색 채우기(명도 차이) — (초안, 위에서 개정됨)
