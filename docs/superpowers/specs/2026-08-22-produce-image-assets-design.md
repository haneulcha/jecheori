...
1. **배경이 평면 마젠타 한 색인가** — 체커보드를 그려주거나(1회차 실패 모드),
   ...
   잡아 실패시키므로(`배경이 제거되지 않았다`) 눈으로 먼저 거른다. 라벨 글자·반짝이
   같은 **부스러기**도 후처리가 잡는다 — 본체에서 떨어진 1% 미만 조각이 있으면
   실패시킨다(2회차 실측: 부스러기는 전부 0.8% 미만, 본체는 95% 이상)
...
## 10. 카드 표지 레이아웃 (이번 범위, 시안 게이트)

96px 도판이 들어오면 표지 행이 다시 짜여야 한다. 현행 `summaryRow`
(`ProduceCard.module.css:32`, `ProduceCard.tsx:43~58`)는 27px 인라인 이모지를
전제로 [이름/품종/제철띠 | 가격]을 좌우로 놓는데, 표식이 27px→96px가 되면 행 높이가
세 배 이상 뛰고 이름·가격·손글씨·마스킹테이프의 균형이 무너진다.

**이건 슬롯 크기 변경이 아니라 레이아웃 개편이다.** 초안 스펙은 이걸 다음 사이클로
미뤄놨는데, 그러면 깨진 중간 상태로 한 사이클을 산다 — 좌우탭→차양 재작업과 같은
실패 모양이다. 이번 범위에 넣는다.

다만 **지금 레이아웃을 지면 위에서 정하지 않는다.** 앵커 4장이 나온 뒤, 진짜 그림을
얹은 표지 시안 2~3안을 만들어 사인오프를 받는다(`CLAUDE.md` UI/UX 결정 규칙).
상상 속 그림에 맞춰 짜면 두 번 짠다.

시안이 지켜야 할 제약:

- 콘텐츠 폭 ≈ 384px (28rem − `#app` 패딩 − 카드 패딩). 96px가 그중 1/4을 먹는다
- 표지에 남아야 할 것: 이름 · 품종(`kind`) · 절정 점(`PeakDot`) · 제철 띠
  (`SeasonStrip`) · 가격 블록(`PriceBlock`, 큰 숫자 + 기준선) · 손글씨 한마디
  (`why`). 하나도 펼침 영역으로 내리지 않는다 — 전부 "펼치기 전에 보이는 것"으로
  설계된 요소다
- 마스킹테이프(`::before`)·홀짝 기울기·crisp 모서리는 유지
- `<summary>` 안이라 클릭 타깃이 행 전체다 — 이미지가 클릭을 삼키지 않아야 한다
- 축산 카드는 `SeasonStrip`이 없다(`ProduceCard.tsx:56`) — 그 경우의 균형도 본다
