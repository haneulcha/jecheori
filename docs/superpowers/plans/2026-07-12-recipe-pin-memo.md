...
- **알려진 소폭 한계(다음 사이클)**: (1) 칩 재탭·카드 접힘 닫기는 즉시(애니메이션 없음) — 압정/Esc만 대칭 닫힘. (2) 스와이프 넘김(§2·§5·§6)은 미도입.
- **최종 리뷰 반영(커밋 b36995e)**: `‹`/`›`가 끝에서 비활성될 때 포커스를 메모로 회수(WCAG 2.4.3), Esc를 포커스-보유 메모로 스코프(여러 카드 동시 열림 시 전부 닫힘 방지), 칩 `aria-controls`를 활성 칩에만 부여. 수용된 Minor: `close()`의 `.chip-btn` DOM 결합, `key={r.name}`, `[index]` effect의 무해한 `setClosing(false)`.
