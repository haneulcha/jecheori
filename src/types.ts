...
/** 계량의 종류. KAMIS 표기(kg·g·근·개·포기·마리·손·장·구·L)는 이 세 종류 중 하나로 떨어진다.
 *
 ...
 *  부류 500(축산물)이 셋째 종류 부피(volume)를 몰고 왔다 — 구(계란 낱개)는 count,
 *  L(우유)은 개당값이 성립하지 않는 volume이다. */
export type Measure =
  | { kind: 'weight'; unit: 'kg' | 'g' | '근' }
  | { kind: 'count'; unit: '개' | '포기' | '마리' | '손' | '장' | '구' }
  | { kind: 'volume'; unit: 'L' }
