...
describe('RecipeMemo', () => {
  ...
  test('첫 레시피에선 이전이, 마지막에선 다음이 비활성 (clamp)', () => {
    ...
    expect(
      within(first.container).getByRole<HTMLButtonElement>('button', { name: '이전 레시피' })
        .disabled,
    ).toBe(true)
    expect(
      within(first.container).getByRole<HTMLButtonElement>('button', { name: '다음 레시피' })
        .disabled,
    ).toBe(false)
    const last = render(<RecipeMemo recipes={recipes} index={2} onClose={noop} onStep={noop} />)
    expect(
      within(last.container).getByRole<HTMLButtonElement>('button', { name: '다음 레시피' })
        .disabled,
    ).toBe(true)
  })
  ...
  test('‹ ›가 onStep(∓1)을 부른다', () => {
    ...
    fireEvent.click(getByRole('button', { name: '이전 레시피' }))
    expect(onStep).toHaveBeenLastCalledWith(-1)
    fireEvent.click(getByRole('button', { name: '다음 레시피' }))
    expect(onStep).toHaveBeenLastCalledWith(1)
  })
  ...
  test('압정 클릭이 닫힘 전환 후 onClose를 부른다', async () => {
    ...
    expect(getByRole('group').hasAttribute('data-closing')).toBe(true)
  ...
  test('닫힘 진행 중 index가 바뀌면 닫힘이 취소되고 onClose가 안 불린다', async () => {
    ...
    expect(getByRole('group').hasAttribute('data-closing')).toBe(false)
