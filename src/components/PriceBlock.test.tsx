...
describe('PriceBlock', () => {
  ...
  test('상승: rise 클래스', () => {
    const { container } = render(
      <PriceBlock price={{ now: 5000, wasMonthAgo: 4400, unit: g100, perUnit: null, change: { kind: 'rise', pct: 14 }, spark: null }} />,
    )
...
describe('PriceBlock 기준선(단위) 줄', () => {
  test('무게 단위: "100g 기준"만', () => {
    const { container } = render(
      <PriceBlock price={{ now: 315, wasMonthAgo: 370, unit: g100, perUnit: null, change: { kind: 'fall', pct: 15 }, spark: null }} />,
    )
  ...
  test('개수 단위(N>1): 기준과 개당값을 한 줄에', () => {
    const { container } = render(
      <PriceBlock price={{ now: 7043, wasMonthAgo: 8009, unit: ten, perUnit: 704, change: { kind: 'fall', pct: 12 }, spark: null }} />,
    )
    const basis = container.querySelector('.basis')
  ...
  test('포기 단수: 기준만, 개당값 없음', () => {
    const { container } = render(
      <PriceBlock price={{ now: 2997, wasMonthAgo: 2911, unit: onePogi, perUnit: null, change: { kind: 'rise', pct: 3 }, spark: null }} />,
    )
  ...
  test('기준선(등락)이 없어도 단위는 남는다 — 단위는 조건부가 아니다', () => {
    ...
    const { container } = render(
      <PriceBlock price={{ now: 19141, wasMonthAgo: null, unit: ten, perUnit: 1914, change: null, spark: null }} />,
    )
